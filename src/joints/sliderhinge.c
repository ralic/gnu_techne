...
@implementation Sliderhinge

-(void) init
{
    self->joint = dJointCreatePR (_WORLD, NULL);
...
-(void) update
{
    ...
    dJointSetPRAnchor (self->joint,
                       self->anchor[0],
                       self->anchor[1],
                       self->anchor[2]);

    dJointSetPRAxis1 (self->joint,
                      self->axes[0][0],
                      self->axes[0][1],
                      self->axes[0][2]);

    dJointSetPRAxis2 (self->joint,
                      self->axes[1][0],
                      self->axes[1][1],
                      self->axes[1][2]);
}
...
-(int) _get_anchor
{
    dJointGetPRAnchor (self->joint, self->anchor);
...
-(int) _get_axes
{
    ...
    dJointGetPRAxis1 (self->joint, self->axes[0]);
    dJointGetPRAxis2 (self->joint, self->axes[1]);
...
-(int) _get_positions
{
    ...
    if (self->joint) {
	state[0] = dJointGetPRPosition (self->joint);
	state[1] = dJointGetPRAngle (self->joint);
...
-(int) _get_rates
{
    ...
    if (self->joint) {
	state[0] = dJointGetPRPositionRate (self->joint);
	state[1] = dJointGetPRAngleRate (self->joint);
